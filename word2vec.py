...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx, :] + np.dot(probs, outsideVectors)
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(scores[outsideWordIdx])) - np.sum(np.log(sigmoids[negSampleWordIndices]))
    
    gradCenterVec = (sigmoid(scores[outsideWordIdx]) - 1) * outsideVectors[outsideWordIdx, :] - \
                    np.sum((sigmoid(-scores[negSampleWordIndices]) - 1)[:, np.newaxis] * outsideVectors[negSampleWordIndices, :], axis=0)
    
    gradOutsideVecs = np.zeros_like(outsideVectors)
    
    gradOutsideVecs[outsideWordIdx, :] = (sigmoid(scores[outsideWordIdx]) - 1) * centerWordVec
    
    for negIdx in negSampleWordIndices:
        gradOutsideVecs[negIdx, :] += (1 - sigmoid(-scores[negIdx])) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        curr_loss, curr_gradCenterVec, curr_gradOutsideVecs = word2vecLossAndGradient(
