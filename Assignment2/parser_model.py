...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ### 
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(hidden_size, self.embed_size * self.n_features))
        ...
        # Initialize hidden_to_logits weights and biases
    ...
    def forward(self, w):
        ...
        hidden_output = torch.matmul(x, self.embed_to_hidden_weight.t()) + self.embed_to_hidden_bias
        hidden_output = F.relu(hidden_output)
        hidden_output = F.dropout(hidden_output, p=self.dropout_prob, training=self.training)
        logits = torch.matmul(hidden_output, self.hidden_to_logits_weight.t()) + self.hidden_to_logits_bias
