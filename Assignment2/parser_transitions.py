...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            # Left Arc: create a dependency from the second topmost word on the stack to the topmost word
            # Remove the topmost word from the stack
            dependent = self.stack.pop(-2)
            if isinstance(dependent, dict):
                   self.dependencies.append((self.stack[-1]["id"], dependent["id"], transition[1:]))
        elif transition.startswith("R"):
            # Right Arc: create a dependency from the topmost word on the stack to the second topmost word
            # Remove the second topmost word from the stack
